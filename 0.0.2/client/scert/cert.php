...
/*
 * cert.php: file for obtaining a certificate on the client side
...
if (isset($_GET['info'])) {
	...
	$url0 = md5(time().$_SERVER['REMOTE_ADDR']);
    $url1 = md5((time() - 1).$_SERVER['REMOTE_ADDR']);
    $url2 = md5((time() - 2).$_SERVER['REMOTE_ADDR']);
    $url3 = md5((time() - 3).$_SERVER['REMOTE_ADDR']);
    $url4 = md5((time() - 4).$_SERVER['REMOTE_ADDR']);
    $url5 = md5((time() - 5).$_SERVER['REMOTE_ADDR']);
    $url6 = md5((time() - 6).$_SERVER['REMOTE_ADDR']);
    $url7 = md5((time() - 7).$_SERVER['REMOTE_ADDR']);
    $url8 = md5((time() - 8).$_SERVER['REMOTE_ADDR']);
    $url9 = md5((time() - 9).$_SERVER['REMOTE_ADDR']);
    $url10 = md5((time() - 10).$_SERVER['REMOTE_ADDR']);

    if ($_GET['info'] == $url0 or 
		$_GET['info'] == $url1 or 
		$_GET['info'] == $url2 or 
		$_GET['info'] == $url3 or 
		$_GET['info'] == $url4 or 
	    ...
	    $iv = json_decode($_GET['4']);
		$certarr = [];
        $cert = str_split(file_get_contents(getcwd()."/cert/cert.scrt"));
		for($i = 0; $i < count($cert); $i++) {
			array_push($certarr, ord($cert[$i]));
		}
	
		$code = explode('.', bcmod(bcpow((string) $_GET['1'], (string) $key, 1000), 
				 $_GET['3'], 1))[0];
		...
		echo json_encode($aescert["cipher"]);
    ...
    $root = rand(3, 20);					   
    $mix = explode('.', bcmod(bcpow((string) $root, (string) $key, 1000), (string) $smNum, 1))[0];
