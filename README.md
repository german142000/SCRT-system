...
Хост сервер и центр сертификации написаны на PHP. Клиенты написаны на java, js и php. На языке java существует две версии - для android и версия для остальных приложений, независящая от android sdk. Клиенты в будущем могут пополняться.

Четвертая версия системы спроектирована под работу в микросервисной архитектуре, однако это не мешает использовать систему в монолитной архитектуре, единственное отличие - 4 версия имеет зависимость от SQL базы данных.
...
Для создания центра сертификации сервер должен поддерживать PHP и иметь модуль openssl. Чтобы центр сертификации заработал, необходимо расположить директорию "*scrt-system_v4*" в любом месте сервера,
...
После загрузки файлов на сервер вам необходимо изменить файл "*config.php*" из папки "*scrt-system_v4*". Он содержит четыре поля:
...
После изменения файла вам необходимо закрыть доступ к папке "*certificates*" из "*scrt-system_v4*". 
...
-   pass - пароль для администрирования, который был указан в файле "*config.php*";
-   sql - если этот параметр установлен в "*true*", помимо файлов сертификата генерируется файл с расширением "*.sql*";

После выполнения запроса на запрашиваемой странице должен появится идентификатор только что созданного сертификата. Его файлы будут лежать по пути "*scrt-system_v4/certificates/{идентификатор сертификата}*".
Каждый сертификат состоит из трёх файлов - "*param.json*", "*public_key.pem*", "*private_key.pem*" и файла "*private_key.sql*", который генерируется по запросу. Чтобы сертификат был действительным в течении выбраного периода, 
в этой папке должны лежать файлы "*param.json*" и "*public_key.pem*". По истечению срока сертификата его директорию можно удалить. SQL запрос из файла "*private_key.sql*" необходимо выполнить к базе данных, из которой берет информацию хост сервер, после чего его 
можно удалить. Файл "*private_key.pem*" использовался в третьей версии системы, сейчас он создается лишь для информативных целей. При желании можно удалить.
...
Для установки требуется PHP с модулями openssl и mysqli, а также настроенная SQL база данных. Чтобы хост заработал, необходимо расположить директорию "*scrt-system_v4_host*" в любом месте сервера,
...
После загрузки файлов на сервер вам необходимо изменить файл "*config.php*" из папки "*scrt-system_v4_host*". Он содержит три поля:

-   version - версия центра сертификации, это поле нельзя менять;
-   scrt_directory - путь к директории scrt хоста на сервере, здесь вам необходимо задать полный путь;
-   password - пароль для администрирования;

Затем вам необходимо настроить доступ к базе данных в файле "*scrt-system_v4_host/sdatabase/main.php*" в строках с 11 по 14.

**Сертификат**

Сертификат должен располагаться в базе данных, доступ к которой настраивался в прошлом пункте. Обратите внимание на параметр "*db_name*" из файла "*scrt-system_v4_host/sdatabase/main.php*". Перед загрузкой сертификата вам необходимо создать базу данных 
с именем, указанном в "*db_name*". После выполнения этих действий, необходимо выполнить SQL запрос из файла "*private_key.sql*" к созданной базе. После этого хост готов к работе.
...
Эти функции прописаны в файле "*packs.php*" в папке "*scrt-system_v4_host*". При написании PHP скрипта вам необходимо подключить этот файл. 
...
`test.php`
```php
include("scrt-system_v4_host/packs.php"); //Подключение SCRT-system
...
- `scrt_cert_address` - адрес центра сертификации с абсолютным путём к папке "*scrt-system_v4*";
- `scrt_host_address` - адрес scrt хоста с абсолютным путём к папке "*scrt-system_v4_host*";
...
что файл по адресу обрабатывает данные с помощью функций "*packs.php*" из директории "*scrt-system_v4_host*", в другом случае поведение не определенно, возвращает объект `SCRT_pack`, который возвращает ответ сервера в расшифрованном виде либо ошибку;

Класс `SCRT_pack` - специальный класс, который позволяет получить как данные, так и ошибку. Имеет в себе только два публичных поля:
...
В этом клиенте используется функциональная парадигма, состоящая из двух функций:
...
что файл по адресу обрабатывает данные с помощью функций "*packs.php*" из директории "*scrt-system_v4_host*", в другом случае поведение не определенно, возвращает отправленные сервером данные в виде строки;
...
Работа с php клиентом похожа на работу с клиентом под java. Для работы используются классы `SCRT_config`, `SCRT_pack` и `SCRT_session`.

Класс `SCRT_config` имеет три публичных поля для настройки:

- `$scrt_cert_address` - адрес центра сертификации с абсолютным путём к папке "*scrt-system_v4*";
- `$scrt_host_address` - адрес к scrt хоста с абсолютным путём к папке "*scrt-system_v4_host*";
- `$keeper` - IP адрес хоста, к которому будет обращаться клиент;

Класс `SCRT_config` используется для начальной настройки объектов класса `SCRT_session`, конструкторы которых принимают объект класса `SCRT_config`.

Класс `SCRT_pack` - специальный класс, который позволяет получить как данные, так и ошибку. Имеет в себе только два публичных поля:

- `$error` - возвращает `true` если в процессе получения ответа возникает ошибка, иначе `false`;
- `$data` - если `error` равен `true` - возвращает текст ошибки, иначе - расшифрованный ответ от хоста;

Для установки соединения с хостом необходимо создать объект класса `SCRT_session`. Этот класс имеет следующие функции:

- `__construct($config)` - конструктор, принимает объект класса `SCRT_config`, возвращает объект `SCRT_session`;
- `getErrorStatus()` - возвращает `true` если в процессе инициализации объекта возникает ошибка, иначе `false`;
- `getError()` - возвращает описание ошибки в виде строки;
- `sendData($url, $data)` - отправляет на хост данные, переданные в виде строки через параметр `$data` по адресу `$url`, ожидается,
что файл по адресу обрабатывает данные с помощью функций "*packs.php*" из директории "*scrt-system_v4_host*", в другом случае поведение не определенно, возвращает объект `SCRT_pack`, который возвращает ответ сервера в расшифрованном виде либо ошибку;
...
while($ss->getErrorStatus()){
    $ss = new SCRT_session($srvcnf);
};

$result = $ss->sendData("http://127.0.0.1/api.php", "data");
if(!$result->error){
    echo "data from server:".$result->data;
} else {
